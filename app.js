...
function restartSession(){
  ...
  $('#hud').classList.add('hidden'); hideHintBar();
}
...
function setHasControls(enable){ const st=document.querySelector('.stage'); if(st){ st.classList.toggle('has-controls', !!enable); } }
...
function render(){
  ...
  setHasControls(mode==='test' || mode==='type'), card=$('#card');
  ...
  $('#hud').classList.add('hidden'); hideHintBar(); $('#mcq').classList.add('hidden'); $('#typePane').classList.add('hidden'); $('#dock').style.display='grid';
...
function renderTestQuestion(){
  ...
  setFrontBack(q); $('#card').classList.remove('flipped'); hideHintBar();
...
function endTest(success){
  ...
  testState=null; $('#hud').classList.add('hidden'); hideHintBar();
...
function renderTypeQuestion(){
  ...
  $('#card').classList.remove('flipped'); hideHintBar();
  ...
  if(ok){
    ...
    slideThen('right',()=>{
      ...
      if(st.idx>=Math.min(TEST_SIZE,st.set.length)){
        alert('จบชุด Type!'); typeState=null; $('#hud').classList.add('hidden'); hideHintBar(); startTypeIfNeeded(); render(); return;
      }
