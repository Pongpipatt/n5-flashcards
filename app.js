...
/***** SLIDE ANIMS (ใช้กับ practice/type feedback) *****/
...
/* ✅ แสดง/ซ่อนหน้า Exam + ซ่อนการ์ด */
function showExam(on){
  const stage = document.querySelector('.stage');
  const exam  = document.getElementById('examPane');
  if(!stage || !exam) return;
  exam.classList.toggle('hidden', !on);
  stage.classList.toggle('has-exam', !!on);
  setHasControls(!!on);
}
/* เผื่อโค้ดเก่าส่งสัญญาณ safe-area — ทำเป็น no-op ป้องกัน error */
function setHasControls(_on){ /* reserved */ }

function hideHintBar(){ $('#hintBar')?.classList.add('hidden'); $('#hintBar')&&( $('#hintBar').classList.remove('show') ); }
...
function render(){
  ...
  if(mode==='test'){
    ...
    $('#dock')?.style && ($('#dock').style.display='none');
    return;
  }
  if(mode==='type'){
    showExam(true);
  ...
  $('#hud')?.classList.add('hidden'); $('#hintBar')?.classList.add('hidden');
  $('#mcq')?.classList.add('hidden'); $('#typePane')?.classList.add('hidden');
  $('#dock')?.style && ($('#dock').style.display='grid');
}
...
function renderTestQuestion(){
  ...
  // Prompt กลาง
  const promptText = (frontKey==='kanji') ? (q.kanji||q.kana||q.meaning)
                    : (frontKey==='kana')  ? (q.kana||q.kanji||q.meaning)
                    : (useThai ? (q.meaningTh||q.meaning) : q.meaning);
  $('#examPrompt').textContent = promptText || '—';

  // แสดง MCQ
  hideHintBar();
  $('#typePane')?.classList.add('hidden');
  ...
  choices.forEach(ch=>{
    ...
    btn.addEventListener('click',()=>{
      ...
      if(ok){
        ...
        btn.classList.add('wrong'); setTimeout(()=>btn.remove(),120);
        showHintBar(()=>{
...
function showHintBar(onUse, used){
  const bar=$('#hintBar');
  if(used){ bar.classList.remove('hidden'); return; }
...
function renderTypeQuestion(){
  ...
  $('#typePane')?.classList.remove('hidden');
  ...
  setTimeout(()=>inputEl?.focus({preventScroll:true}),30);
}
